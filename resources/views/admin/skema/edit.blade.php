...
<div class="max-w-3xl mx-auto lg:px-[40px] md:px-[24px] px-[16px] py-8">
  <div class="mb-6">
    <h1 class="text-2xl font-semibold">Edit Skema</h1>
    <p class="text-sm text-gray-600">Perbarui data skema.</p>
  </div>
  ...
  <form action="{{ route('admin.skema.update', $skema) }}" method="POST" class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-5">
    @csrf
    @method('PUT')

    <div>
      <label class="block text-sm font-medium text-gray-700">Kode Skema</label>
      <input type="text" name="kode_skema" value="{{ old('kode_skema', $skema->kode_skema) }}" class="mt-1 w-full rounded-lg border-gray-300 focus:border-teal-500 focus:ring-teal-500">
    </div>

    <div>
      <label class="block text-sm font-medium text-gray-700">Nama Skema</label>
      <input type="text" name="nama_skema" value="{{ old('nama_skema', $skema->nama_skema) }}" class="mt-1 w-full rounded-lg border-gray-300 focus:border-teal-500 focus:ring-teal-500">
    </div>
    ...
    <div>
      ...
      <select name="kategori_id" class="mt-1 w-full rounded-lg border-gray-300 focus:border-teal-500 focus:ring-teal-500" required>
        @foreach($kategoris as $k)
          <option value="{{ $k->id }}" {{ old('kategori_id', $skema->kategori_id) == $k->id ? 'selected' : '' }}>{{ $k->nama_kategori }}</option>
        @endforeach
      </select>
      @error('kategori_id')
    ...
    <div class="flex items-center justify-end gap-2">
      ...
      <button type="submit" class="inline-flex px-4 py-2 rounded-lg bg-teal-600 text-white hover:bg-teal-700">Simpan Perubahan</button>
    </div>
  </form>
</div>
@endsection
...
<script>
  (function(){
    ...
    const initialSub = '{{ old('subkategori_id', $skema->subkategori_id ?? '') }}';
