...
@section('title', 'Edit Berita | LSP CMS')
...
<a href="javascript:history.back()" class="inline-flex items-center gap-2 text-gray-600 text-sm mb-4">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-4 h-4"><path d="M10.828 12l4.95-4.95a1 1 0 10-1.414-1.414l-6.364 6.364a1 1 0 000 1.414l6.364 6.364a1 1 0 001.414-1.414L10.828 12z"/></svg>
    Kembali
</a>

<h1 class="text-3xl font-semibold mb-4">Edit Berita</h1>
                ...
                <div class="space-y-5">
                    <div>
                        ...
                        <input type="text" name="judul" id="judul"
                            ...
                            class="block w-full rounded-lg border border-gray-300 bg-white shadow-sm px-3 py-2
                                   focus:border-black focus:ring-1 focus:ring-black sm:text-sm" required>
                        @error('judul')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                    </div>

                    <div>
                        <label for="isi" class="block text-sm font-medium text-gray-700 mb-1">Isi <span class="text-red-500">*</span></label>
                        <textarea name="isi" id="isi" rows="6"
                            class="block w-full rounded-lg border border-gray-300 bg-white shadow-sm px-3 py-2
                                   focus:border-black focus:ring-1 focus:ring-black sm:text-sm" required>{{ old('isi', $berita->isi) }}</textarea>
                        @error('isi')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                    </div>

                    <div>
                        <label for="sumber" class="block text-sm font-medium text-gray-700 mb-1">Penulis <span class="text-red-500">*</span></label>
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal" id="tanggal"
                            value="{{ old('tanggal', $berita->tanggal) }}" max="{{ now()->toDateString() }}"
                            class="block w-full rounded-lg border border-gray-300 bg-white shadow-sm px-3 py-2
                                   focus:border-black focus:ring-1 focus:ring-black sm:text-sm" required>
                    ...
                    <div class="rounded-xl border border-gray-200 p-4">
                        ...
                        <div class="rounded-xl border-2 border-dashed border-gray-300 bg-gray-50 overflow-hidden">
                            <label class="block relative cursor-pointer min-h-[220px]">
                                <div id="uploadPlaceholder" class="absolute inset-0 flex flex-col items-center justify-center p-10 text-center {{ $berita->gambar ? 'hidden' : '' }}">
                                ...
                                <img id="preview" class="absolute inset-0 w-full h-full object-cover {{ $berita->gambar ? '' : 'hidden' }}" alt="Preview" src="{{ $berita->gambar ? asset('storage/'.$berita->gambar) : '' }}">
